...
def _bo(vi: list[str], en: list[str]) -> DistractorSet:
    return DistractorSet(vi_options=vi, en_options=en)


def _bo_hop_le() -> DistractorSet:
    return _bo(
...
def _bo_tu_payload(payload: Any) -> DistractorSet:
...
def test_bo_du_ba_phan_tu_moi_chieu_khong_trung_khong_dung_dap_an_dung_thi_hop_le() -> None:
    """Ca nền: bộ sạch phải được nhận. Không có test này thì một validator luôn trả False
    vẫn làm mọi test còn lại xanh."""
    assert is_valid(_bo_hop_le(), MEANING, TERM) is True


def test_thieu_phan_tu_thi_loai() -> None:
    """Hai mồi nhử = câu trắc nghiệm chỉ có ba lựa chọn, xác suất đoán bừa nhảy từ 25% lên
    33%."""
    assert (
        is_valid(
            _bo(["làm trầm trọng thêm", "phóng đại"], ["aggravate", "exaggerate", "postpone"]),
            MEANING,
            TERM,
        )
        is False
...
def test_thua_phan_tu_thi_loai() -> None:
    ...
    assert (
        is_valid(
            _bo(["a", "b", "c"], ["aggravate", "exaggerate", "postpone", "delay"]),
            MEANING,
            TERM,
        )
        is False
...
def test_phan_tu_rong_hoac_chi_co_khoang_trang_thi_loai() -> None:
...
def test_hai_phan_tu_trung_nhau_trong_cung_mot_chieu_thi_loai() -> None:
    """So sau khi chuẩn hoá (strip + lowercase): "phóng đại" và "Phóng Đại" là MỘT lựa chọn
    hiện hai lần, dù chuỗi khác nhau."""
    assert (
        is_valid(
            _bo(["phóng đại", "Phóng Đại", "trì hoãn"], ["aggravate", "exaggerate", "postpone"]),
            MEANING,
            TERM,
        )
        is False
...
def test_moi_nhu_trung_nghia_dung_thi_loai() -> None:
    ...
    assert (
        is_valid(
            _bo(["  Giảm Nhẹ ", "phóng đại", "trì hoãn"], ["aggravate", "exaggerate", "postpone"]),
            MEANING,
            TERM,
        )
        is False
...
def test_moi_nhu_trung_chinh_tu_dang_hoi_thi_loai() -> None:
...
def test_thieu_field_hoac_field_khong_phai_mang_thi_loai_khong_nem_loi() -> None:
    ...
    """
    assert is_valid(_bo_tu_payload({}), MEANING, TERM) is False
    assert is_valid(_bo_tu_payload({"en_options": ["a", "b", "c"]}), MEANING, TERM) is False
    assert (
        is_valid(
            _bo_tu_payload({"vi_options": "không phải mảng", "en_options": ["a", "b", "c"]}),
            MEANING,
            TERM,
        )
        is False
    )
    assert is_valid(_bo_tu_payload("cả payload không phải object"), MEANING, TERM) is False


def test_phan_tu_khong_phai_chuoi_lam_hong_ca_bo_khong_nem_loi() -> None:
    ...
    assert (
        is_valid(
            _bo_tu_payload({"vi_options": ["a", None, "c"], "en_options": ["x", "y", "z"]}),
            MEANING,
            TERM,
        )
        is False
    ...
    assert (
        is_valid(
            _bo_tu_payload({"vi_options": ["a", "b", "c"], "en_options": ["x", 7, "z"]}),
            MEANING,
            TERM,
        )
        is False
...
def test_dap_an_dung_la_none_thi_khong_nem_loi_va_bo_van_hop_le() -> None:
    ...
    trước rồi.
    """
    assert is_valid(_bo_hop_le(), None, None) is True
