...
/**
 * Bản giả của đường xác thực extension: có token trong tay, và 401 thì xoá nó đi.
 *
 * Thay cho `chrome.storage` + `auth-storage` mà bộ test này dùng trước khi tách package.
 * Ý nghĩa từng khẳng định giữ nguyên: `creds.token === null` ở đây đúng bằng
 * `loadAuth() === null` ngày trước.
 */
...
describe('ApiClient', () => {
  ...
  beforeEach(async () => {
    ...
    client = new ApiClient(() => Promise.resolve(BASE_URL), creds);
