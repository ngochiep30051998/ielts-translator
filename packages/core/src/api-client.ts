...
export class ApiClient {
  ...
  constructor(
    private readonly baseUrlProvider: () => Promise<string>,
  ...
  private async request<T>(path: string, init: RequestInit,
    ...
    if (authenticated) {
      const headers = await this.credentials.authHeaders();
      if (headers === null) {
        ...
        //
        // CHỈ extension đi được vào nhánh này. Web không đọc được cookie httpOnly nên
        // không có cơ sở khẳng định "chưa đăng nhập" — nó luôn trả header và để server
        // quyết định.
    ...
    if (response.status === 401 && authenticated) {
      ...
      await this.credentials.onUnauthorized();
    }
